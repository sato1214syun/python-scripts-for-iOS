...
def ImageDownload(img_info_dict, tempFileName, isUseIcab=False):
    if isUseIcab:
        downloadByIcab(img_info_dict, tempFileName)
    else:
        downloadByPython(img_info_dict, tempFileName)


def downloadByPython(img_info_dict, tempFileName):
...
def downloadByIcab(img_info_dict, temp_file_name):
    print('画像urlをicab mobileに転送中...')
    title = img_info_dict["title"]
    num_of_pages = img_info_dict["num_of_pages"]
    num_of_remaining_pages = len(img_info_dict["url_ext_list"])
    x_head = 'x-icabmobile://x-callback-url/download?'
    scheme = ''
    params = {'x-success': 'pythonista:'}
    if num_of_pages == num_of_remaining_pages: #ページ総数と画像データ数が一致(新規ダウンロードの場合)
        clipboard.set(title)
        webbrowser.open('x-icabmobile://x-callback-url/nomalmode')
        time.sleep(1)
        while console.is_in_background():
            time.sleep(1)
    
    # 画像URLのリストから実際の画像をダウンロードする
    start_page_no = num_of_pages - num_of_remaining_pages
    copy_dict = copy.copy(img_info_dict["url_ext_list"])
    for i, (url, ext) in enumerate(copy_dict.items(), start=start_page_no):
        if i == num_of_pages - 1:
            del params['x-success']
            
        del img_info_dict["url_ext_list"][url]
        page_no = "{0:03d}".format(i + 1)
        file_name = '{}.{}'.format(page_no, ext)
        
        params['url'] = url
        params['filename'] = file_name
        scheme = x_head + urllib.parse.urlencode(params, quote_via=urllib.parse.quote)
        webbrowser.open(scheme)
        
        if not i == num_of_pages - 1: 
            time.sleep(0.7)
            # while console.is_in_background():
            #     time.sleep(1)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(temp_file_name):
        ...
        ImageDownload(img_info_dict, tempFileName)
